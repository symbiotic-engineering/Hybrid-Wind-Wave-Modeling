...
omega_min =6.9*pi/30;                       % Minimun rotor speed (only used for ROSCO controller) [rad/s]
omega_rated_try=12.1*pi/30;                 % Nominal rotor rated speed [m/s]
max_Thrust_factor=0.8;                      % Maximum thrust force factor (max thrust with peak shaving/max thrust without peak shaving)
Prated=5.3e6;                               % Rated power [W]
...
cd control
load("Properties_5MW.mat");   % Load windturbine properties
load("Bladedata_5MW.mat");   % Load blade data
...
%% SAVE
save('SteadyStates_5MW','SteadyStates')
    ...
    for bl=1:3
        ...
        for node=1:length(data.r)
            ...
            while eps>data.root_tol_rel  &&  i<=data.maxit
                ...
                Alpha = data.airfoil(:,1,node);  % Alpha (deg)
                Cl = data.airfoil(:,2,node);     % Coefficient of lift
                Cd = data.airfoil(:,3,node);     % Coefficient of drag
                Cm = data.airfoil(:,4,node);     % Coefficient of moment
                
                % Remove duplicates
                [AlphaUnique, uniqueIdx] = unique(Alpha, 'stable'); % Keep unique rows
                ClUnique = Cl(uniqueIdx);
                CdUnique = Cd(uniqueIdx);
                CmUnique = Cm(uniqueIdx);
                
                % Perform interpolation
                CL = interp1(AlphaUnique, ClUnique, alfa);
                CD = interp1(AlphaUnique, CdUnique, alfa);
                CM = interp1(AlphaUnique, CmUnique, alfa);
